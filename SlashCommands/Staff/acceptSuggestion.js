...
module.exports = {
    name: 'accept-suggest',
    description: 'Accetta un suggerimento di un utente',
    ...
    options: [
      {
        name: 'id_messaggio',
        description: 'ID del messaggio del suggerimento da accettare',
        type: 'STRING',
        required: true,
      },
      {
        name: 'motivo',
        description: 'Motivo dell\'accettamento',
        type: 'STRING',
        required: false,
      },
    ],
    ...
    run: async (bot, interaction, args) => {
        ...
        .setColor("GREEN")
        .addField(`**__Stato Suggerimento:__**`, `**\`Suggerimento Accettato\`**`)
        .addField(`**__Commento Approvatore:__**`, grund)
        ...
        interaction.reply({ content: "**Suggerimento accettato con successo!**" });
    
            const user = await bot.users.cache.find(
                (u) => u.tag === data.author.name
            );
            user.send({ content: `**Il tuo suggerimento è stato accettato!**\n**Autore: ${interaction.user}**\n**Commento Autore: \`${grund}\`**` });
        } catch (err) {
            console.log(err)
