...
module.exports = {
    ...
    try{
      ...
      if(!args[0]) return message.reply({ content: "Perfavore, menziona o inserisci l'id id un utente valido per potergli mandare una risposta all'idea inviatoci!" })
      if(!utente) return message.reply({ conten: "Questo utente non esiste. Specifica un utente valido!" })
      
      const msg = args.slice(1).join(" ");
      if(!msg) return message.reply({ content: "Perfavore, inserisci un messaggio come risposta idea!" })

        const embed = new Discord.MessageEmbed()
        .setTitle("**__Risposta Idea__**")
        .setDescription(`${msg}\n\n**\`- ${message.author.tag} | Developer\`**`)
        .setColor("GREEN")
        .setFooter("Messaggio da parte del team developer Los Angeles Full RP Bot")
        user.send({ embeds: [embed] })

        message.channel.send({ content: `**${message.author} risposta idea inviata con successo all'utente!**` })
        canale.send({ content: `**E' appena stata inviata una risposta idea ad un utente!\n\n\`Autore:\` ${message.author}\n\`Utente:\` ${user} ( \`${user.id}\` )\n\`Messaggio:\`** \`\`\`${msg}\`\`\`` })
      } catch (e) {
        if (
          String(e).includes(
